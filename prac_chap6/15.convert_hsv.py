...
def calc_hsi(bgr):
    ...
    angle = math.acos(tmp1 / tmp2) * (180 / np.pi) if tmp2 else 0  # 각도 계산

    H = angle if B <= G else 360 - angle                           # 색상 계산
    S = 1.0 - 3 * min([R, G, B]) / bgr_sum if bgr_sum else 0       # 채도 계산
    I = bgr_sum / 3                                                # 명도 계산
    return (H/2, S*255, I)

# BGR 컬러 -> HSI 컬러
def bgr2hsi(image):
    hsv = [[calc_hsi(pixel) for pixel in row] for row in image ]   # 받은 image의 2차원 배열 순회, HSI 계산
    return (np.array(hsv)).astype('uint8') # 계산한 HSI 값 반환

BGR_img = cv2.imread("image/color_space.jpg", cv2.IMREAD_COLOR) # 컬러 영상 읽기
if BGR_img is None: raise Exception("영상 파일 읽기 오류")

HSI_img = bgr2hsi(BGR_img)                  # BGR를 HSI로 변환
HSV_img = cv2.cvtColor(BGR_img, cv2.COLOR_BGR2HSV) # OpenCV 함수로 HSV 변환, HSI 계산과 수식이 조금 다름
