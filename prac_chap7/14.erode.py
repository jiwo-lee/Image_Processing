...
def erode(img, mask=None):
    ...
    mcnt = cv2.countNonZero(mask) # 마스크에서 1인인 원소 개수를 세서 mcnt에 저장
    for i in range(ycenter, img.shape[0] - ycenter):           # 입력 행렬 반복 순회
        for j in range(xcenter, img.shape[1] - xcenter):
            ...
            cnt  =  cv2.countNonZero(temp) # 관심영역과 마스크가 일치한 화소수를 cnt에 저장
            dst[i, j] = 255 if (cnt == mcnt) else 0 # cnt == mcnt면 i,j에 255를, 아니면 0을 저장
    return dst
